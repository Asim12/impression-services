...
Class Mongo_db {
    ...
    //Convert a object to  Mongo ObjectId
    public function mongoId($mongoID) {

        if ($this->isValid($mongoID)) {
    ...
    public function is_valid_mongo_id($mongoID) {
    ...
    function isValid($value) {
        ...
        try {
            ...
            show_error("Wrong  MongoDB ObjectID", 500);
        }
    }
    ...
    public function insert($collection = "", $insert = array()) {
        ...
        try
        {
            ...
            $result = $this->db->{$collection}->insertOne($insert);
    ...
    public function where($wheres, $value = null) {
        if (is_array($wheres)) {
            foreach ($wheres as $wh => $val) {
                if ($wh == '_id' || $wh == 'buy_order_id' || $wh == 'order_id') {
                    $this->wheres[$wh] = new MongoDB\BSON\ObjectID($val);
                } else {
                    $this->wheres[$wh] = $val;
                }
            ...
            if ($value == '_id' || $value == 'buy_order_id' || $value == 'order_id') {
    ...
    public function regex(string $regex = '', string $flags = 'i', bool $enable_start_wildcard = TRUE, bool $enable_end_wildcard = TRUE) {
        ...
        require 'mongodb_class/Regex.php';
    ...
    public function get($collection = "") {
        ...
        //  $documents = $this->db->{$collection}
        // ->find($this->wheres, $this->selects,[ 'limit' =>1 ]);
...
//return $collection->find([], [ 'limit' => 2 ]);
     ...
     * @usage : $this->mongo_db->count('foo');
     */
    public function countData($collection = "") {
        if (empty($collection)) {
            show_error("In order to retrieve a count of documents from MongoDB, a collection name must be passed", 500);
        }
    ...
    //Function for mongodate format

    public function converToMongodttime($datetime) {
    ...
    /**
     * --------------------------------------------------------------------------------
     * //! List all indexes of Collection / Table
     * --------------------------------------------------------------------------------
     *
     * List all indexes
     *
     ...
     */
    public function list_indexes_new($collection = "") {
        if (empty($collection)) {
        ...
        try
        {
            ...
            if (isset($this->debug) == TRUE && $this->debug == TRUE) {
                show_error("Get indexes from MongoDB failed: {$e->getMessage()}", 500);
            } else {
                show_error("Get indexes from MongoDB failed", 500);
            }
        }
    }
    ...
    //Functin for show all Collections
    public function show_All_Collections() {

        $list = $this->db->listCollections();
        $data = iterator_to_array($list);
        $all_collecion_arr = array();
        if (!empty($data)) {
            foreach ($data as $row) {
                $new_2 = (array) $row;
                foreach ($new_2 as $row_1) {
                    array_push($all_collecion_arr, $row_1['name']);
                }
            }
        }
        return $all_collecion_arr;
    } //End show_All_Collections
    ...
    public function getCustomeQuery($collection, $operation) {
        if (empty($collection)) {
            show_error("In order to retreive documents from MongoDB, a collection name must be passed", 500);
        }
        ...
        try
        {
            ...
            $documents = $this->db->{$collection}->aggregate($operation);
            /*$collection = $this->db->selectCollection($collection);
            $documents    = $collection->aggregate($operation);*/
            foreach ($documents as $doc) {
                echo '<pre>';
                print_r($doc);
            }
    ...
    /*
public function sum_aggregate()
{
try
{
$collection = $this->db->selectCollection( 'sentiments_tweet' );

$sql = "SELECT COUNT(*) as totalrecord, SUM(negative_sentiment) as negative_sentiment ,SUM(positive_sentiment) as positive_sentiment

FROM `tweets` WHERE `keyword` = '".$_GET['keyword']."' AND `created_at` BETWEEN ".$from." AND ".$timestampOnDay."";
...
$where['fk_user_id'] = '5af17c361c0b760bc966436a';
$order_by['store_id'] = 1;
$start=0;
$limit=2;
